...
import kots.mq.{Destination, MqMetrics}
...
object PrometheusMqMetrics {
  ...
  def register[F[_]](
    ...
    F.delay {
      val publishedTotal = counter(registry, "kots_mq_published", "messages published")
      val receivedTotal = counter(registry, "kots_mq_received", "deliveries received")
      val redeliveredTotal =
        counter(registry, "kots_mq_redelivered", "deliveries past their first attempt")
      val acknowledgedTotal = counter(registry, "kots_mq_acknowledged", "deliveries acknowledged")
      val rejectedTotal = counter(registry, "kots_mq_rejected", "deliveries rejected")
      val publishSeconds = histogram(registry, "kots_mq_publish_latency_seconds", "publish latency")
      val receiveSeconds = histogram(registry, "kots_mq_receive_latency_seconds", "receive latency")
      val ackSeconds = histogram(registry, "kots_mq_time_to_ack_seconds", "time to acknowledge")
