...
final class PrometheusMqMetricsSuite extends CatsEffectSuite {
  ...
  test("every signal of the port reaches the registry") {
    ...
    for {
      ...
      assertEquals(counterValue(registry, "kots_messaging_published"), 1.0d)
      assertEquals(counterValue(registry, "kots_messaging_received"), 2.0d)
      assertEquals(counterValue(registry, "kots_messaging_redelivered"), 1.0d)
      assertEquals(counterValue(registry, "kots_messaging_rejected"), 1.0d)
      assertEquals(counterValue(registry, "kots_messaging_acknowledged"), 1.0d)
      assertEquals(histogramCount(registry, "kots_messaging_publish_latency_seconds"), 1L)
      assert(histogramCount(registry, "kots_messaging_receive_latency_seconds") >= 2L)
      assertEquals(histogramCount(registry, "kots_messaging_time_to_ack_seconds"), 1L)
    }
  }
  ...
  test("a destination labels its own series") {
    ...
    for {
      ...
      val labels = registry
        ...
        .asScala
        .collectFirst {
          case snapshot: CounterSnapshot if snapshot.getMetadata.getName == "kots_messaging_published" =>
